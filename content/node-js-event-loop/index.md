...
## Node.js의 구조
![](index/1*yEW6321eqBd_-C0D7LsBQw.png)
**이벤트 루프**를 이해하기 위해서 `Node.js`가 어떻게 구성되어 있는지를 알아야 한다. `Node.js`는 `C++`로 작성된 런타임이고 그 내부에 `V8 Engine`를 가지고 있다. 그 덕분에 크롬과 같은 브라우저에서 실행하던 자바스크립트를 로컬에서 실행할 수 있다. 그런데 그 내부에는 `V8 Engine` 말고도 `libuv `라는 라이브러리가 존재한다.
### libuv
![](index/banner.png)
`libUV`란 `C++`로 작성된, `Node.js`가 사용하는 **비동기 I/O** 라이브러리다. 이는 사실 운영체제의 **커널**을 추상화한 Wrapping 라이브러리로 **커널**이 **어떤 비동기 API**를 지원하는지 알고있다. 
![](index/libuv%20%E1%84%8F%E1%85%A5%E1%84%82%E1%85%A5%E1%86%AF.png)
...
![](index/A434392B-912E-49FF-A24B-EF9131E63B0A.png)
> 실제로 `node`를 실행하면 그 아래의 여러 개의 `node` 스레드가 존재하는 것을 확인할 수 있다.  
![](index/libuv%20%E1%84%8B%E1%85%AF%E1%84%8F%E1%85%A5%20%E1%84%89%E1%85%B3%E1%84%85%E1%85%A6%E1%84%83%E1%85%B3%201.png)
...
![](index/libuv%20%E1%84%8B%E1%85%AF%E1%84%8F%E1%85%A5%20%E1%84%89%E1%85%B3%E1%84%85%E1%85%A6%E1%84%83%E1%85%B3%202.png)
...
### 그래서 `libuv`가 뭔데?
지금까지 `libuv`라는 비동기 I/O 라이브러리가 존재하고 `Node.js`가 이를 내부적으로 이용한다는 사실을 살펴봤다. 그렇다면 도대체 `libuv`와 **이벤트 루프**는 어떠한 관계가 있고 그래서 `Node.js`는 어떻게 싱글 스레드로 **논블로킹 비동기** 작업을 지원하는걸까?
`Node.js`는 **I/O 작업**을 자신의 **메인 스레드**가 아닌 **다른 스레드**에 위임함으로써 싱글 스레드로 **논 블로킹 I/O**를 지원한다. 다르게 말하면 `Node.js`는 **I/O 작업**을 `libuv`에게 위임함으로써 **논 블로킹 I/O**를 지원하고 그 기반에는 **이벤트 루프**가 있다.
## Nods.js 속 이벤트 루프
![](index/event-loop.png)
...
![](index/Node.js%20Event%20Loop%20Execute%20Basic.png)
...
![](index/Node.js%20Event%20Loop%20Execute%20Callback%201.png)
...
![](index/Node.js%20Event%20Loop%20Execute%20Callback%202.png)
* `Node.js`가 열심히 **이벤트 루프**를 돌다가 `A` 페이즈에 진입한다
*  `Node.js`는 `Q`를 확인한다. 
* `Q`에서 `F1`를 꺼내서 실행한다.
	* `SELECT * FROM SPRING_BOOT`라는 새로운 쿼리를 날린다
![](index/Node.js%20Event%20Loop%20Execute%20Callback%203.png)
* `Q`에서 `F1`를 꺼내서 실행한다.
	* 콘솔에 쿼리 결과를 출력하기 전에 `SELECT * FROM SPRING_BOOT`의 응답이 와서 `F2` 콜백을 `Q`에 추가한다
	* 콘솔에 `SELECT * FROM EVENT_LOOP` 결과를 출력한다.
![](index/Node.js%20Event%20Loop%20Execute%20Callback%204.png)
* `Node.js`는 `Q`를 확인한다.
* `Q`에서 `F2`를 꺼내서 실행한다.
...
## Node 실행과 이벤트 루프 흐름
지금까지 **이벤트 루프**를 구성하는 **페이즈**와 **큐**는 무엇인지, `Node.js`가 여러 페이즈를 돌아다니면서 무슨 일을 하는지 간단하게 알아봤다. 각 **페이즈**는 자신들이 **관심있어 하는 작업**들만 자신의 큐에서 관리한다. 예를 들어 `Timer Phase`는 이름 그대로 **타이머**에 관한 비동기 작업들을 관리한다. `Close Callback Phase`는 `Close Callback`과 관련된 비동기 작업들만 관리한다.
![](index/Event%20Loop%20Flow.png)
...
```
![](index/Event%20Loop%20Flow%20Explain%201.png)
`node test.js`를 실행하면 `Node.js`는 우선 **이벤트 루프**를 생성한다.
![](index/Event%20Loop%20Flow%20Explain%202.png)
...
![](index/Event%20Loop%20Flow%20Explain%203.png)
...
![](index/Event%20Loop%20Flow%20Explain%204.png)
...
![](index/Event%20Loop%20Flow%20Explain%201%202.png)
`node test.js`를 실행하면 `Node.js`는 우선 **이벤트 루프**를 생성한다.
![](index/Event%20Loop%20Flow%20Explain%2010.png)
...
![](index/Event%20Loop%20Flow%20Explain%203%202.png)
...
![](index/Event%20Loop%20Flow%20Explain%205.png)
...
![](index/Event%20Loop%20Flow%20Explain%206.png)
...
![](index/Event%20Loop%20Flow%20Explain%209.png)
...
`Poll Phase`에서 살펴봤듯이 `Node.js`가 `Poll Phase`에서 얼마나 기다리는 지에 영향을 미친다.
...
![](index/nextTickQueue%20order%20change.png)
